...
#let thesis(
  // Status of the document. Just a draft, or the final version.
  // ti.e. show/hide the red TODO text and the current timestamp "Preview printed 20xx-xx-xx" on the title page
  draft: true,
  // We default to a BA/MA thesis, but creating an expose is supported
  expose: false,
  // Currently supported abbreviations: 'ims', 'swd', 'itm', 'msd', 'mi'
  study: "<study>",
  // Supported: 'en' and 'de'
  ...
  // we shorten dois to display only the number
  // (but still have the link to doi.org)
  show "https://doi.org/": w => { // handle DOIs
    [DOI:] + str.from-unicode(160) // 160 A0 nbsp
  }


  // we shorten arxiv links if provided as URL
  // Note: there might by a version number such as v1, v2 (suffix)
  // https://arxiv.org/abs/2402.05709v1 <= 2024-02-08
  // https://arxiv.org/abs/2402.05709v2 <= 2025-09-29
  show "https://arxiv.org/abs/": w => { // handle DOIs
    [ArXiv:] + str.from-unicode(160) // 160 A0 nbsp
  }
  // TODO: if converted, e.g.
  // https://arxiv2bibtex.org/?q=2402.05709&format=bibtex
  // eprint archive-specific electronic identifier
  // @misc{Wei:2025, ...
  //   eprint     = "arXiv:2402.05709",
  // }
  // or
  // @misc{Wei:2025, ...
  //   eprint     = "2402.05709",
  ...
  if (study == "ims") {
    ...
    todo([
      ...
      Configuration value for <study> can be 'ims', 'swd', 'msd', or 'mi'.
    ...
    header: context {
      ...
      if headingsFoundOnCurrentPageWithNumber.len() > 0 {
        ...
        if lastHeading.numbering != none { // normal chapters
          let show_number = counter(selector).get().first()+1;
          //show_number
          if language == "en" {
            ht-last.update([ Chapter #show_number #currSectionName])
          }else{
            ht-last.update([ Kapitel #show_number #currSectionName])
          }
        }
  ...
  todo([
    ...
    TODO: Specify the title, subtitle, author, submission date, study, language,
      ...
      with _typst compile thesis.typ_. Finally, remove all _TODOs_ (todo marcos)
      and the "Preview printed" text from within your typst source code.
  ])

  // Before submitting the final version, remove preview/timestamp:
  align(center + bottom,text(red,[Preview printed #datetime.today().display().]))
  }
  pagebreak()
  if(after-title != none){
    after-title
...
*/
    ...
    show outline.entry.where(level: 1): it => {
        v(0.2em) // more beautiful with a bit more space after subheadings
        it
    }
    // better stick to a maximum of three levels
    ...
    if show-list-of.contains("listings") {
      pagebreak()
      if (language == "de") {
        heading("Quellcodeverzeichnis", numbering: none)
      } else {
