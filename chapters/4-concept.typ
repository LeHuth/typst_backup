...
Die Anfrageschnittstelle bildet die externe Zugriffsschicht des Routing-Kerns. Sie nimmt Pfadanfragen in Form von Start- und Zielkoordinaten entgegen, leitet sie an das jeweils gewählte Suchverfahren weiter und gibt den resultierenden Pfad an den Konsumenten zurück. Sie kapselt die internen Datenstrukturen und stellt die Suchverfahren unabhängig vom konkreten Konsumenten bereit. Konsument im laufenden Betrieb ist der interaktive Client. Der Evaluationsbetrieb umgeht die Anfrageschnittstelle bewusst und greift direkt auf die Suchverfahren zu, um Messungen frei von Transport- und Serialisierungs-Overhead zu halten. Die Anfrageschnittstelle bedient ausschließlich Pfadanfragen; lesende Zugriffe der Visualisierungskomponente auf die Zwischenzustände des Routing-Kerns erfolgen über separate Beobachtungsschnittstellen, deren Datenflüsse im Diagramm als direkte Kanten zwischen Visualisierungs- und Routing-Komponenten dargestellt sind.

Zwei weitere Komponenten dienen ausschließlich der vergleichenden Auswertung, die im Titel dieser Arbeit angekündigt ist. Die Inspektions- und Visualisierungskomponente macht die Zwischenzustände der Routing-Kern-Komponenten über dedizierte Lese- und Stream-Schnittstellen sichtbar: die Cluster-Zuordnung, die Verteilung der Gate Nodes und der Fortschritt der Suchexpansion lassen sich über eine kartenbasierte Darstellung beobachten. Diese Sichtbarkeit ist Voraussetzung für die qualitative Beurteilung des Algorithmenverhaltens jenseits aggregierter Kennzahlen. Die Benchmark- und Evaluationskomponente ist die treibende Komponente der quantitativen Auswertung. Sie ist mit beiden Suchverfahren gekoppelt, steuert sie wiederholt mit denselben Eingaben an und aggregiert die anfallenden Laufzeit- und Qualitätsmetriken zu einer nach Pfadlängenklassen aufgeschlüsselten Statistik. Beide Auswertungskomponenten gehören nicht zum produktiven Routing-Kern, sind aus konzeptioneller Sicht aber integraler Bestandteil des Systems, da die im Titel angekündigte vergleichende Untersuchung ohne sie nicht durchführbar wäre.
...
#diagram(
    ...
    node-inset: 10pt,
    spacing: (5em, 2em),
    // Phasen-Beschriftungen (ueber jeder Spalte, ohne Rahmen)
    node((0, -0.7), [
      #set align(center)
      *Build-Phase* \
    ...
    // Build-Phase (linke Spalte, top -> bottom)
    node((0, 0), [OSM-Extrakt], name: <osm>),
    node((0, 1), [Basisgraph], name: <bg>),
    node((0, 2), [Voronoi-Partition], name: <vor>),
    node((0, 3), [Gate Nodes], name: <gates>),
    node((0, 4), [Abstrakter Graph], name: <ag>),
    ...
    // Datenuebergabe zwischen Phasen (gestrichelt, Label auf der Linie)
    edge(<ag>, <asearch>, "-|>",
